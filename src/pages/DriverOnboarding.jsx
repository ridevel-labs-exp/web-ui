...
import { Upload, CheckCircle2, AlertTriangle, Play, Pause, Power, Navigation, FileText, Check } from 'lucide-react';
...
export default function DriverOnboarding() {
  ...
  const toggleOnline = async () => {
    // In local development, we toggle online status by pinging telemetry location directly
    const nextOnlineState = !isOnline;
    try {
      // Send location update to make them available in PostGIS
  ...
  // Start simulating vehicle movement along the route coordinates pings
  const startLocationSimulation = () => {
    if (simulating) return;
    setSimulating(true);
    simulationStepRef.current = 0;

    // Send first coordinate update
    ...
    }, 4000); // Send telemetry updates every 4 seconds
  };
                    ...
                    {assignedTrips.map(trip => (
                      <div key={trip.id} style={{ border: '1px solid var(--border-glass)', borderRadius: '12px', padding: '16px', background: 'rgba(255,255,255,0.01)' }}>
                        ...
                        <div style={{ display: 'flex', gap: '10px' }}>
                          ...
                          {trip.status === 'ARRIVED' && (
                            <button className="btn-primary" style={{ padding: '8px 16px', fontSize: '13px', flexGrow: 1 }} onClick={() => handleUpdateStatus(trip.id, 'STARTED')}>
                              Start Trip
                            </button>
                          )}
                          {trip.status === 'STARTED' && (
